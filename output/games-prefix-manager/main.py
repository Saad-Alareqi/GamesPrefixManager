        ...
        try:
            ...
            if "appname" in current and "appid" in current:
                ...
                if isinstance(exe, bytes):
                    exe = exe.decode("utf-8", errors="ignore")
                startdir = current.get("startdir", "")
                if isinstance(startdir, bytes):
                    startdir = startdir.decode("utf-8", errors="ignore")
                
                exe_clean = exe.strip('"').strip("'")
                startdir_clean = startdir.strip('"').strip("'")
                
                if startdir_clean and startdir_clean != "-":
                    install_path = startdir_clean
                elif exe_clean and exe_clean != "-":
                    install_path = os.path.dirname(exe_clean)
                else:
                    install_path = "-"
...
class Plugin:
    ...
    async def delete_prefix(self, appid, do_backup, delete_game_folder=False):
        ...
        try:
            ...
            if delete_game_folder and install_path != "-" and os.path.lexists(install_path):
                logger.info(f"Deleting game folder: {install_path}")
                clean_path = os.path.abspath(install_path)
                forbidden = ["/", "/home/deck", "/home/deck/Desktop", "/home/deck/Downloads", "/home/deck/Documents", "/run/media/deck"]
                if clean_path in forbidden or clean_path.startswith("/usr") or clean_path.startswith("/var"):
                    logger.error(f"Refusing to delete forbidden path: {clean_path}")
                else:
                    import subprocess
