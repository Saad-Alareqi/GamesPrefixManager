(function (deckyFrontendLib, React) {
    ...
    const ConfirmModal = ({ title, message, onConfirm, closeModal, showDeleteGameFolder = false, gamePath }) => {
        const [deleteGameFolder, setDeleteGameFolder] = React.useState(false);
        return (React__default["default"].createElement(deckyFrontendLib.ModalRoot, { onCancel: closeModal },
            React__default["default"].createElement("div", { style: { padding: "20px" } },
                ...
                showDeleteGameFolder && (React__default["default"].createElement("div", { style: { marginBottom: "20px" } },
                    React__default["default"].createElement(deckyFrontendLib.ToggleField, { label: "Delete Game Data", description: "Deletes the game install directory alongside the prefix.", checked: deleteGameFolder, onChange: (val) => setDeleteGameFolder(val) }),
                    gamePath && (React__default["default"].createElement("div", { style: { marginTop: "5px", padding: "8px", backgroundColor: "rgba(0,0,0,0.2)", borderRadius: "4px", fontSize: "0.85em", color: "#aaa", wordBreak: "break-all" } },
                        React__default["default"].createElement("strong", null, "Path:"),
                        " ",
                        gamePath)))),
    ...
    const PrefixManagerModal = ({ serverAPI, closeModal }) => {
        ...
        const handleDelete = (game) => {
            deckyFrontendLib.showModal(React__default["default"].createElement(ConfirmModal, { title: "Delete Prefix", message: `Delete prefix for ${game.name} (AppID: ${game.appid})?\nSize: ${game.size}\nStatus: ${game.prefix_status}`, closeModal: () => { }, showDeleteGameFolder: game.install_path !== "-" && !!game.install_path, gamePath: game.install_path, onConfirm: async (backup, deleteGameFolder) => {
                    setLoading(true);
                    await serverAPI.callPluginMethod("delete_prefix", {
                        appid: game.appid,
                        do_backup: backup,
